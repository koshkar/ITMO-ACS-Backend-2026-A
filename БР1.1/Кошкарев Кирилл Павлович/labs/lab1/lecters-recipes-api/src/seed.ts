...
import {
  ...
  RecipeMedia, RecipeStatus, RecipeStep, Tag, User, UserRole, Comment, Like, Follow,
} from './models';
...
const seed = async (): Promise<void> => {
  ...
  console.log('[seed] соединение установлено');

  await AppDataSource.transaction(async (manager) => {
    // --- очистка ---
    await manager.query(
      'TRUNCATE TABLE follows, favorites, likes, comments, recipe_tags, recipe_media, ' +
        'recipe_steps, recipe_ingredients, recipes, ingredients, tags, cuisines, categories, users ' +
    ...
    const cuisines = await manager.save(
      CUISINES.map((name) => manager.create(Cuisine, { name, slug: slugify(name) })),
    );
    const tags = await manager.save(
      TAGS.map((name) => manager.create(Tag, { name, slug: slugify(name) })),
    );
    const ingredients = await manager.save(
      INGREDIENTS.map(([name, defaultUnit, kcal]) =>
        manager.create(Ingredient, {
          name: name as string,
          defaultUnit: defaultUnit as string,
    ...
    console.log(
      `[seed] справочники: ${categories.length} категорий, ${cuisines.length} кухонь, ` +
        `${tags.length} тегов, ${ingredients.length} ингредиентов`,
    );

    // --- пользователи ---
    const password = await hashPassword('Chianti2026!');
    const [lecter, crawford, graham, admin] = await manager.save([
      manager.create(User, {
        email: 'hannibal@lecters.recipes',
        username: 'dr_lecter',
      ...
      manager.create(User, {
        email: 'jack@lecters.recipes',
        username: 'jack_crawford',
        passwordHash: password,
        fullName: 'Jack Crawford',
        bio: 'Ценю простую еду и крепкий кофе.',
        avatarUrl: '/images/avatars/jack_crawford.png',
      }),
      manager.create(User, {
        email: 'will@lecters.recipes',
        username: 'will_graham',
        passwordHash: password,
        fullName: 'Will Graham',
        bio: 'Готовлю редко, но с полным погружением.',
        avatarUrl: '/images/avatars/will_graham.png',
      }),
      manager.create(User, {
        email: 'admin@lecters.recipes',
        username: 'admin',
        passwordHash: password,
    ...
    console.log('[seed] пользователей: 4 (пароль у всех — Chianti2026!)');
    ...
    const data: SeedRecipe[] = [
      {
        author: lecter,
        title: 'Оссо буко по-милански',
        summary: 'Томлёная телячья голяшка с белым вином и цитрусовой густой подливой.',
        description:
          'Классика ломбардской кухни. Требует терпения: мясо томится почти два часа, ' +
          'и только тогда соединительная ткань превращается в шелковистый соус.',
        category: cat('Основные блюда'),
        cuisine: cui('Итальянская'),
        difficulty: RecipeDifficulty.HARD,
        prep: 30,
        cook: 120,
        servings: 4,
        calories: 620,
        tags: [tag('ужин'), tag('изысканное'), tag('мясо'), tag('вино')],
        ingredients: [
      ...
      },
      {
        ...
        category: cat('Закуски'),
        cuisine: cui('Французская'),
        difficulty: RecipeDifficulty.MEDIUM,
        prep: 20,
        cook: 25,
        servings: 6,
        calories: 310,
        tags: [tag('изысканное'), tag('праздничное')],
        ingredients: [
        ...
        media: [
          ['/images/pate.jpg', MediaType.PHOTO, 'Паштет в форме с подачей'],
        ],
      },
      {
        author: lecter,
        ...
        media: [
          ['/images/risotto.jpg', MediaType.PHOTO, 'Ризотто со спаржей'],
        ],
      },
      {
        author: graham,
        ...
        servings: 2,
        calories: 480,
        tags: [tag('праздничное'), tag('быстро')],
        ingredients: [
        ...
        media: [
          ['/images/fondant.jpg', MediaType.PHOTO, 'Разрез фондана'],
        ],
      },
      {
        author: crawford,
    ...
    for (const item of data) {
      ...
      await manager.save(
        item.ingredients.map(([name, quantity, unit, note], index) =>
          manager.create(RecipeIngredient, {
            recipeId: recipe.id,
            ingredientId: ing(name).id,
      ...
      await manager.save(
        item.steps.map(([instruction, duration], index) =>
      ...
      if (item.media.length) {
        await manager.save(
          item.media.map(([url, type, caption], index) =>
    ...
    console.log(`[seed] рецептов: ${savedRecipes.length}`);

    // --- социальные связи ---
    await manager.save([
      manager.create(Follow, { followerId: graham.id, followingId: lecter.id }),
      manager.create(Follow, { followerId: crawford.id, followingId: lecter.id }),
      manager.create(Follow, { followerId: lecter.id, followingId: graham.id }),
    ]);

    const likeRows = [
      { userId: graham.id, recipeId: savedRecipes[0].id },
      { userId: crawford.id, recipeId: savedRecipes[0].id },
      { userId: graham.id, recipeId: savedRecipes[1].id },
      { userId: lecter.id, recipeId: savedRecipes[3].id },
    ];
    await manager.save(likeRows.map((row) => manager.create(Like, row)));
    for (const recipe of savedRecipes) {
      const count = likeRows.filter((row) => row.recipeId === recipe.id).length;
      if (count) await manager.update(Recipe, recipe.id, { likesCount: count });
    }

    const commentRows = [
      { recipeId: savedRecipes[0].id, authorId: graham.id, body: 'Два часа того стоили. Соус получился плотным.' },
      { recipeId: savedRecipes[0].id, authorId: crawford.id, body: 'Готовил в чугунной утятнице — отлично.' },
      { recipeId: savedRecipes[1].id, authorId: crawford.id, body: 'Подавал с бриошью, как и советовали.' },
    ];
    await manager.save(commentRows.map((row) => manager.create(Comment, row)));
    for (const recipe of savedRecipes) {
      const count = commentRows.filter((row) => row.recipeId === recipe.id).length;
      if (count) await manager.update(Recipe, recipe.id, { commentsCount: count });
    }

    console.log('[seed] социальные связи созданы');
    console.log(`[seed] администратор: admin@lecters.recipes / Chianti2026! (id=${admin.id})`);
  });
