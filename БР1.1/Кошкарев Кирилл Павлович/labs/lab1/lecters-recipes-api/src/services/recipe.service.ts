...
import {
  Category, Cuisine, Favorite, Ingredient, Like, Recipe, RecipeIngredient, RecipeMedia,
  RecipeStatus, RecipeStep, UserRole,
} from '../models';
import {
  CreateRecipeInput, RecipeQuery, UpdateRecipeInput,
} from '../dto';
import { z } from 'zod';
import { recipeIngredientInputSchema, recipeMediaInputSchema, recipeStepInputSchema } from '../dto';
...
const assertExists = async (id: string | null | undefined, repo: 'category' | 'cuisine') => {
  if (id === null || id === undefined) return;
  const found =
    repo === 'category'
      ? await categories().findOne({ where: { id: String(id) } })
      : await cuisines().findOne({ where: { id: String(id) } });
  if (!found) {
    throw notFound(repo === 'category' ? 'Категория не найдена' : 'Кухня не найдена');
  }
};

export const recipeService = {
  /** Поиск рецептов с фильтрацией — основной сценарий витрины. */
  async search(query: RecipeQuery, params: PageParams, viewerId?: string): Promise<Paginated<Recipe>> {
    ...
    // Черновики видит только их автор, остальным — только опубликованные рецепты.
    ...
    if (query.q) {
      qb.andWhere('(recipe.title ILIKE :q OR recipe.summary ILIKE :q OR recipe.description ILIKE :q)', {
        q: `%${query.q}%`,
      });
    }
    ...
    if (query.maxCookTime !== undefined) {
      qb.andWhere('recipe.cookTimeMinutes <= :maxCookTime', { maxCookTime: query.maxCookTime });
    }
    if (query.minCookTime !== undefined) {
      qb.andWhere('recipe.cookTimeMinutes >= :minCookTime', { minCookTime: query.minCookTime });
    }
    if (query.ingredientId?.length) {
      // рецепт должен содержать ВСЕ перечисленные ингредиенты
      qb.andWhere(
  ...
  /** Возвращает рецепт с проверкой права на просмотр черновика. */
  async getForViewer(id: string, viewerId?: string, viewerRole?: string): Promise<Recipe> {
    ...
    if (recipe.status !== RecipeStatus.PUBLISHED && !isOwner && viewerRole !== UserRole.ADMIN) {
  ...
  assertCanModify(recipe: Recipe, userId: string, role?: string): void {
    if (String(recipe.authorId) !== userId && role !== UserRole.ADMIN) {
      throw forbidden('Операция доступна только автору рецепта');
    }
  },

  async viewerState(recipeId: string, viewerId?: string) {
    if (!viewerId) return { isLiked: false, isFavorite: false };
    const [like, favorite] = await Promise.all([
      likes().findOne({ where: { recipeId, userId: viewerId } }),
      favorites().findOne({ where: { recipeId, userId: viewerId } }),
    ]);
  ...
  async create(authorId: string, input: CreateRecipeInput): Promise<Recipe> {
    await assertExists(input.categoryId as string | null | undefined, 'category');
    await assertExists(input.cuisineId as string | null | undefined, 'cuisine');

    const repo = recipes();
    const slug = await uniqueSlug(
      input.title,
      async (candidate) => Boolean(await repo.findOne({ where: { slug: candidate } })),
    );
  ...
  async update(recipe: Recipe, input: UpdateRecipeInput): Promise<Recipe> {
    ...
    if (input.title !== undefined && input.title !== recipe.title) {
      ...
      recipe.slug = await uniqueSlug(
        input.title,
        async (candidate) =>
          Boolean(
            await repo
              .createQueryBuilder('r')
              .where('r.slug = :candidate AND r.id <> :id', { candidate, id: recipe.id })
              .getOne(),
          ),
      );
    }
    ...
    if (input.categoryId !== undefined) {
      recipe.categoryId = input.categoryId ? String(input.categoryId) : null;
    }
    if (input.cuisineId !== undefined) {
      recipe.cuisineId = input.cuisineId ? String(input.cuisineId) : null;
    }
    ...
    if (input.status !== undefined) {
      ...
      if (input.status === RecipeStatus.PUBLISHED && !recipe.publishedAt) {
  ...
  async publish(recipe: Recipe): Promise<Recipe> {
    const stepsCount = await recipeSteps().count({ where: { recipeId: recipe.id } });
    if (stepsCount === 0) {
      throw unprocessable('Нельзя опубликовать рецепт без шагов приготовления');
    }
    const ingredientsCount = await recipeIngredients().count({ where: { recipeId: recipe.id } });
  ...
  async replaceIngredients(recipeId: string, items: IngredientInput[]): Promise<RecipeIngredient[]> {
    ...
    if (items.length) {
      ...
      if (found.length !== ids.length) {
        throw notFound('Один или несколько ингредиентов не найдены в справочнике');
      }
    }
    await repo.delete({ recipeId });
    if (!items.length) return [];

    // дубликаты ингредиента внутри рецепта запрещены уникальным индексом
  ...
  async listIngredients(recipeId: string): Promise<RecipeIngredient[]> {
    return recipeIngredients().find({ where: { recipeId }, order: { position: 'ASC' } });
  },

  async replaceSteps(recipeId: string, items: StepInput[]): Promise<RecipeStep[]> {
    const repo = recipeSteps();
    const numbers = new Set(items.map((item) => item.stepNumber));
    if (numbers.size !== items.length) {
  ...
  async listSteps(recipeId: string): Promise<RecipeStep[]> {
    return recipeSteps().find({ where: { recipeId }, order: { stepNumber: 'ASC' } });
  },

  async listMedia(recipeId: string): Promise<RecipeMedia[]> {
    return recipeMedia().find({ where: { recipeId }, order: { position: 'ASC' } });
  },

  async addMedia(recipeId: string, input: MediaInput): Promise<RecipeMedia> {
  ...
  /** Лента подписок: опубликованные рецепты авторов, на которых подписан пользователь. */
  async feed(authorIds: string[], params: PageParams): Promise<Paginated<Recipe>> {
